...
from flaskDemo.forms import RegistrationForm, LoginForm, UpdateAccountForm, PostForm, WorkerForm,WorkerUpdateForm, EmployeeLookup, LoginForm, RoleForm,RoleUpdateForm,updateSSN
...
def map_home():
    mydata = get_schedule()
    coordinates = {'lat': 41.8847, 'lng': -87.6291}
    return render_template('map_home.html', coordinates=coordinates, results=mydata)
...
def home():
    ...
    if currentPerson.PersonType == 'Admin':
        ...
        return render_template('adminhome.html', title='Home', mydata=mydata)
    else:
        mydata=get_schedule()
...
def adminAbout():
    return render_tevmplate('adminAbout.html', title='About')
...
def new_worker():
    ...
    if form.validate_on_submit():
        ...
        PhoneNumber = form.ph58oneNumber.data, Address = form.address.data, email = form.email.data,Instrument = form.instrument.data,Title = form.title.data)
...
def rolePage(id,roleid):
   ...
   # PerformanceDate = request.args.get('PerformanceDate', None)
   # PersonID = request.args.get('PersonID', None)
   # RoleID = request.args.get('RoleID', None)
   # PerformanceTime = request.args.get('PerformanceTime', None)
   # RoleName = request.args.get('RoleName', None)
   # Latitude = request.args.get('Latitude', None)
   # Longitude = request.args.get('Longitude', None)
   # print(Longitude)
   # print(RoleID)
   # res = ast.literal_eval(mydata)
...
def update_role(id,roleid):
    ...
    if form.validate_on_submit():
        ...
        role.PerformanceID = form.performanceID.data
        db.session.commit()
        ...
        form.performanceID.data = role.PerformanceID
    return render_template('roles.html', title='Update Workers',
