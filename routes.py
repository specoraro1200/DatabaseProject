...
from flaskDemo.forms import RegistrationForm, LoginForm, UpdateAccountForm, PostForm, DeptForm,DeptUpdateForm, EmployeeLookup, LoginForm, RoleForm
...
def selected(id):
    ...
    return render_template('dept.html', title=worker.FirstName, worker=worker, join = roles, now=datetime.utcnow(), a = current_user.id, b = worker.id)
  #  form = DeptForm()
  #  if form.validate_on_submit():
...
def new_assign():
    ...
    if form.validate_on_submit():
       # assign = Person(id=form.id.data, FirstName=form.firstname.data, MInit = form.middlename.data, LastName = form.lastname.data, PersonType = form.title.data, DateHired = form.hired.data, \
        #PhoneNumber = form.phoneNumber.data, Address = form.address.data, username = form.username.data, password = form.password.data, Instrument = form.instrument.data)
        db.session.add(assign)
        db.session.commit()
